...
OUT_DIR="${OUT_DIR:-$ROOT_DIR/dist/testpypi}"
...
# Editable installs leave generated caches in-tree.
...
if [[ "$UPLOAD" != "1" ]]; then
  ...
  echo "set UPLOAD=1 and TEST_PYPI_API_TOKEN to upload to TestPyPI"
  exit 0
fi

if [[ -z "${TEST_PYPI_API_TOKEN:-}" ]]; then
  echo "TEST_PYPI_API_TOKEN is required when UPLOAD=1" >&2
  exit 1
fi
